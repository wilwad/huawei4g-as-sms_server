...
function get_dhcp_settings( csrftoken, cbSuccess, cbFail ){
		if (! cookie_.trim().length) {
		...
		let route = huawei4G.routes.dhcp_settings
		...
		request({
			...
			if ( obj.DhcpIPAddress !== undefined ) {
				cbSuccess( obj )
			} else {
				cbFail( 'obj.DhcpIPAddress == undefined' )
			}
		});
}
