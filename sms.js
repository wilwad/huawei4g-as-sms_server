...
function sms_initBoxList() {
    ...
    $.each(g_sms_smsList, function(n,message) {
        ...
        +"<td class='sms_phone_number'>"+messagePhone+"</td>"
        +"</tr>"
        +"</table>");
        $row.append($cell_2);
        //
...
function sms_showWaitingDialog(tipTitle, tipContent, callback_func) {
    ...
    "<tr><td class='wait_table_header'><label class='wait_title clr_white'>" +
    tipTitle +
    "</label><span class='wait_dialog_btn' id='wait_dialog_btn clr_gray' ><img src='../res/dialog_close_btn.png' alt='' /></span></td></tr>" +
    ...
    function sms_createSms(btnValue) {
        ...
        if (strPhoneNumber.length > 0) {
            if (strPhoneNumber.lastIndexOf(";") == (strPhoneNumber.length - 1)) {
                ...
                if(g_contact_enabled == 1 && g_module.pb_enabled) {                    
                    $("#recipients_number").val(sendInformation);
                }else {                    
...
function editInformation() {
    ...
    sendInformation = $('#recipients_number').val();
    if(sendInformation == '' || sendInformation == null) {
        judgeFlag = true;
        $('#recipients_number').val('');        
    }
    if(!judgeFlag) {
        ...
        $('#recipients_number').val(sendInformations);
    }         
}   
    $('#search_recipients_number').live('click',function() {
        var displayInformations = '';
    	displayInformations = $("#recipients_number").val();
    	...
    	$('.linkmanInfo').bind('click',function() {    	    
            ...
            $('#recipients_number').val(displayInformations);             		
    	});
    	
    	$('#recipients_number').live('click',function() {
    		$('#user_add_select').hide();
    		$('#search_recipients_number').show();
            $('#close_search_recipients_number').hide();    		    		
    	});
...
{
    var phonebook = g_pb_contactListArray;
